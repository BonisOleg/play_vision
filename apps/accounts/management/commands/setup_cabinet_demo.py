...
class Command(BaseCommand):
    help = 'Setup demo data for cabinet testing'

    def add_arguments(self, parser):
        parser.add_argument(
            '--email',
            type=str,
            default='demo@playvision.com',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Створення демо даних для {email}...')
        ...
        self.setup_user_profile(user)
        
        # 6. Створити активну підписку
        self.create_user_subscription(user)
        
        # 7. Створити аккаунт лояльності
        self.setup_loyalty_account(user)
        
        # 8. Створити демо курси та матеріали
        self.create_demo_courses(user)
        
        self.stdout.write(
            self.style.SUCCESS(f'✅ Демо дані успішно створено для {email}')
        )
        self.stdout.write(f'🔗 Увійдіть в аккаунт та перейдіть до /account/')
    ...
    def create_loyalty_tiers(self):
        ...
        for tier_data in tiers_data:
            ...
            if created:
                self.stdout.write(f'  ✅ Створено рівень: {tier.name}')
    ...
    def create_subscription_plans(self):
        ...
        for plan_data in plans_data:
            ...
            if created:
                self.stdout.write(f'  ✅ Створено план: {plan.name}')
    ...
    def create_demo_user(self, email):
        ...
        if created:
            ...
            user.save()
            self.stdout.write(f'  ✅ Створено користувача: {user.email}')
            self.stdout.write(f'  🔑 Пароль: demo123456')
        else:
            self.stdout.write(f'  ⚠️ Користувач вже існує: {user.email}')
        
        return user

    def setup_user_profile(self, user):
        ...
        if created:
            ...
            self.stdout.write(f'  ✅ Створено профіль для користувача')

    def create_user_subscription(self, user):
        ...
        if created:
            self.stdout.write(f'  ✅ Створено активну підписку: {plan.name}')

    def setup_loyalty_account(self, user):
        ...
        if created:
            ...
            self.stdout.write(f'  ✅ Налаштовано аккаунт лояльності: {loyalty_account.current_tier.name}')

    def create_demo_courses(self, user):
        """Створити демо курси та матеріали"""
        from apps.content.models import UserCourseProgress, Favorite
        ...
        for course_data in courses_data:
            ...
            if created:
                self.stdout.write(f'  ✅ Створено курс: {course.title}')
        ...
        self.stdout.write(f'  ✅ Створено демо курси та прогрес')
